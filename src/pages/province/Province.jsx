...
export const Province = () => {
  ...
  const discardProvince = () => {
    setForm(initialForm);
    setEditingId(null);
  };

  const getProvinces = async (page, limit) => {
    try {
      const provinceService = new ProvinceService();
      const response = await provinceService.getProvinces(page, limit);
      setProvinces(response?.data || []);
      setPagination(response?.pagination || null);
    } catch (error) {
      console.error("Error en getProvinces:", error.message);
      setProvinces([]);
      setPagination(null);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const provinceService = new ProvinceService();
      if (editingId) {
        await provinceService.updateProvince(editingId, form);
        alert("Provincia actualizada correctamente");
      } else {
        await provinceService.createProvince(form);
        alert("Provincia creada correctamente");
      }
      discardProvince();
      getProvinces(page, limit);
    } catch (error) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold">Gestor de provincias</h2>
      <p className="text-slate-500">
        Administre las provincias y sus datos para una correcta gestión de envíos.
      </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <div className="flex justify-end items-center gap-3">
            ...
            >
              <Save size={18} />
              {editingId ? "Actualizar Provincia" : "Crear Provincia"}
            </button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  type="text"
                  placeholder="Ej. Región Pampeana, Patagonia, etc."
                  required
                  className="w-full px-2 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#6366f1] focus:border-[#6366f1]"
                />
              </div>
      ...
      <div className="w-full flex gap-3 mt-4">
        <div className="w-full bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <h2 className="text-lg font-semibold mb-6">Listado de provincias</h2>
                  ...
                  provinces.map((province) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        >
                          {province.active === 1 ? "Activo" : "Inactivo"}
                        </span>
                      </td>
      ...
      {pagination && (
        <div className="sticky bottom-0 w-full flex justify-between gap-4 mt-6 bg-white border border-gray-200 px-6 py-4 z-10">
          ...
          <div className="flex items-center">
            <span className="text-gray-500 font-semibold">
              Mostrando {provinces.length} de {pagination.total} provincias
            </span>
          </div>
        </div>
      )}
