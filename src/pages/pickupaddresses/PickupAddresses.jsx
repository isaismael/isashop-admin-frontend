import { Save, Trash, SquarePen, Frown, MapPin } from "lucide-react";
import { useState, useEffect } from "react";
import PickupAddress from "../../services/pickupaddress.service";
...
export const PickupAddresses = () => {
  ...
  const getPickupAddresses = async (page, limit) => {
    try {
      const service = new PickupAddress();
      const response = await service.getPickupAddresses(page, limit);
      setPickupAddresses(response?.data || []);
      setPagination(response?.pagination || null);
    } catch (error) {
      console.error("Error en getPickupAddresses:", error.message);
      setPickupAddresses([]);
      setPagination(null);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const service = new PickupAddress();
      if (editingId) {
        await service.updatePickupAddress(editingId, form);
        alert("Punto de recogida actualizado correctamente");
      } else {
        await service.createPickupAddress(form);
        alert("Punto de recogida creado correctamente");
      }
      discard();
      getPickupAddresses(page, limit);
    } catch (error) {
      console.error("Error al guardar punto de recogida:", error);
    }
  };
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold">Gestor de puntos de recogida</h2>
      <p className="text-slate-500">
        Administre los puntos de recogida desde donde se retirarán los productos.
      </p>
      ...
      <div className="w-full flex gap-3 mt-4">
        <div className="w-full bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <h2 className="text-lg font-semibold mb-6">Listado de puntos de recogida</h2>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Dirección</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Estado</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Acción</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {Array.isArray(pickupAddresses) && pickupAddresses.length > 0 ? (
      ...
      {pagination && (
        <div className="sticky bottom-0 w-full flex justify-between gap-4 mt-6 bg-white border border-gray-200 px-6 py-4 z-10">
          ...
          <div className="flex items-center">
            <span className="text-gray-500 font-semibold">
              Mostrando {pickupAddresses.length} de {pagination.total} puntos de recogida
            </span>
          </div>
        </div>
      )}
